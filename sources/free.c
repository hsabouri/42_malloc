...
/*   Updated: 2018/01/29 17:33:34 by hsabouri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_free_bucket(t_pool *pool, size_t i)
{
	...
	ft_store(buckets[i].mem, HIST_FREE, buckets[i].size, pool->sbucket);
	buckets[i].size = 0;
	tmp = buckets[i];
...
void	ft_del_pool_first(t_pool **lst)
{
	t_pool	*tmp;
